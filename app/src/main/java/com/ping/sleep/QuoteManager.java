...
public class QuoteManager {
    ...
    public static void loadQuotes(Context context, SharedPreferences prefs) {
        quotes.clear();
        String path = prefs.getString("quote_path", getDefaultQuotePath(context));
    ...
    private static String getDefaultQuotePath(Context context) {
        // 使用应用私有目录，避免硬编码外部存储路径
        File internalDir = new File(context.getFilesDir(), "quotes");
        if (!internalDir.exists()) {
            internalDir.mkdirs();
        }
        return new File(internalDir, "quotes.txt").getAbsolutePath();
    }
